...
fs.readdir('./commands/', (err, files) => { 
  ...
  console.log(`${files.length} komut yüklenecek.`)//Broken
  files.forEach(f => {                    
...
client.on('message', async message => {
  ...
  if(message.content === '.tag') {
    message.channel.send(`\`${ayarlar.tag}\``)//Broken
  }
  })
...
const arw = `Ve senin hesabın sunucumuza kayıt olmak için tüm şartları karşılıyor! ${client.emojis.cache.get(ayarlar.yes)}`//Broken
if(arwZaman < 1296000000) {
  arw = `Ve senin hesabın sunucumuza kayıt olmak için daha çok genç! ${client.emojis.cache.get(ayarlar.no)}`//Broken
}
  
member.roles.add(ayarlar.kayıtsızRol)
member.roles.add(ayarlar.kayıtsızRol)
member.roles.add(ayarlar.kayıtsızRol)//Broken
